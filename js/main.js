...
// Opening Website
 const launchDate = new Date("August 27, 2025 09:20:00").getTime();
  const popup = document.getElementById("launchPopup");
  const countdown = document.getElementById("countdownTimer");
  const devBypassKey = "letmeinwild"; // 🔑 Your secret passkey here
  const bypassKeyName = "EchoedWildDevBypass";

  function checkDevKey() {
    const input = document.getElementById("devKey").value;
    const message = document.getElementById("devMessage");
    if (input === devBypassKey) {
      localStorage.setItem(bypassKeyName, "true");
      message.textContent = "✅ Developer mode enabled. Reloading...";
      setTimeout(() => location.reload(), 1000);
    } else {
      message.textContent = "❌ Incorrect passkey.";
    }
  }
  ...
  // Check time and dev bypass
  window.addEventListener("DOMContentLoaded", () => {
    const now = new Date().getTime();
    const bypass = localStorage.getItem(bypassKeyName);
